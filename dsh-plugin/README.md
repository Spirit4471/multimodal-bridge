...
| `qwen_generate(prompt, size, n, negative_prompt)` | 文生图，保存为本地 PNG | Qwen-Image 系列（`qwen-image-2.0` / `wanx2.1-t2i-turbo` …） |
...
| `outputDir` | `generated` | 生成图保存目录；相对路径基于会话工作区（启动目录）解析 |
...
- **生成自动路由（fallback）**：免费额度 ≠ 网关路由表——工作空间网关只挂载部分
  ...
  `generateFallbackModels` 顺序自动换模型重试；401/403 等认证/权限错误不转移，
  直接失败返回。
